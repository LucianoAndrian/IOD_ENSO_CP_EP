...
def Aux_SelectEvents(f, var_file, cases_dir, data_dir, out_dir,
    ...
    if case_events is not None:
        logger.debug('Set case data done')

        if obs_var is not None:
            logger.debug('Adding obs data')
            ...
            logger.debug('Concat case data with obs data')
