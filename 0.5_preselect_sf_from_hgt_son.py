"""
Test: streamfunction from geopotential height SON
...
# ---------------------------------------------------------------------------- #
logger = init_logger('0.5_preselect_sf_from_hgt_son.log')
...
def hgt_to_psi(data):
    ...
    psi = g / np.transpose(np.tile(f, (nlons, 1))) * data

    return psi


data_1982_1998 = hgt_to_psi(data_1982_1998)
data_1999_2011 = hgt_to_psi(data_1999_2011)
...
SaveNC(son_hindcast, f'{out_dir}{v_name}_aux_hindcast_no_detrend_son.nc')

logger.info('Saving: detrend...')
SaveNC(son_hindcast_detrend, f'{out_dir}{v_name}_aux_hindcast_detrend_son.nc')

logger.info('Saving: clim...')
SaveNC(son_clim_99_11, f'{out_dir}{v_name}_aux_son_clim_99_11.nc')
...
son_clim_99_11 = xr.open_dataset(f'{out_dir}{v_name}_aux_son_clim_99_11.nc')
son_hindcast_no_detrend = \
    xr.open_dataset(f'{out_dir}{v_name}_aux_hindcast_no_detrend_son.nc')
son_hindcast_detrend = \
    xr.open_dataset(f'{out_dir}{v_name}_aux_hindcast_detrend_son.nc')
...
if save_nc:
    ...
    logger.info('Saving: no detrend...')
    SaveNC(son_f, f'{out_dir}{v_name}_son_no_detrend.nc')

    logger.info('Saving: detrend...')
    SaveNC(son_f_detrend, f'{out_dir}{v_name}_son_detrend.nc')
