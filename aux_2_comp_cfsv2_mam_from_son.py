...
if save:
    ...
    dpi = 100
# ---------------------------------------------------------------------------- #
logger = init_logger('aux_2_comp_cfsv2_MAM_from_son.log')
...
def OpenSetCases(var, idx1, idx2, idx3, phase, dir, sig=False):
    ...
    # neutro
    if sig is False:
        cases['neutros'] = xr.open_dataset(f'{dir}{var}_neutros_MAM_from_SON.nc') * fix
    ...
    for i_num, i in enumerate(indices):
        ...
        try:
            cases[i]['puros'] = xr.open_dataset(
                f'{dir}{var}_puros_{i}_{phase}_MAM_from_SON.nc') * fix
        ...
        try:
            ...
            triple = xr.open_dataset(
                ...
                f'_{phase}_MAM_from_SON.nc') * fix
