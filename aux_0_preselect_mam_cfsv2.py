...
for v in variables:
    ...
    mam_hindcast = xr.concat([mam_anom_82_98, mam_anom_99_11], dim='time')

    mam_hindcast_detrend = xr.concat(
    ...
    mam_clim_99_11 = xr.open_dataset(f'{out_dir}{v}_aux_mam_clim_99_11.nc')
    mam_hindcast_no_detrend = \
        xr.open_dataset(f'{out_dir}{v}_aux_hindcast_no_detrend_mam.nc')
    mam_hindcast_detrend = \
        xr.open_dataset(f'{out_dir}{v}_aux_hindcast_detrend_mam.nc')

    mam_realtime_no_detrend = Anom_SeasonRealTime(data, mam_clim_99_11, 4)
    logger.info('realtime_no_detrend loading')
    mam_realtime_no_detrend.load()

    mam_realtime_detrend = Anom_Detrend_SeasonRealTime(
        data, mam_clim_99_11, 4)
    mam_realtime_detrend.load()

    logger.info('Concat hindcast + realtime')
    mam_f = xr.concat(
        [mam_hindcast_no_detrend, mam_realtime_no_detrend], dim='time')
    mam_f_detrend = xr.concat(
        [mam_hindcast_detrend, mam_realtime_detrend], dim='time')
