...
def ClasificarEventos(r, ds1, ds2, ds3, variables, thr=0.5):
    ...
    eventos['todo'] = {
        index1: {
            'pos': ds1.time.where(i1 > 0).dropna('time'),
            'neg': ds1.time.where(i1 < 0).dropna('time')
        },
        index2: {
            'pos': ds1.time.where(i2 > 0).dropna('time'),
            'neg': ds1.time.where(i2 < 0).dropna('time')
        },
        index3: {
            'pos': ds1.time.where(i3 > 0).dropna('time'),
            'neg': ds1.time.where(i3 < 0).dropna('time')
        }
    }
